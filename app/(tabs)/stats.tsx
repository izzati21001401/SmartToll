...
export default function StatsPage() {
    ...
    const fetchLicensePlate = async () => {
      try {
        ...
        if (!userSnapshot.empty) {
          ...
          console.log("No license plate found for user.");
        }
      } catch (error) {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      ...
      try {
        console.log("Fetching transactions for plate:", licensePlate);

        const transactionsRef = collection(db, "tollRecords");
        ...
        transactionSnapshot.forEach((doc) => {
          ...
          const tollFee = parseFloat(data.tollFee) || 0;

          total += tollFee;
          if (transactionDate >= currentMonthStart) {
            monthlyTotal += tollFee;
          }
        });
        ...
        <View style={styles.cardsContainer}>
          <View style={styles.card}>
            ...
            <Text style={styles.cardValue}>
              RM {monthlyExpenses.toFixed(2)}
            </Text>
          </View>
          <View style={styles.card}>
            <Text style={styles.cardTitle}>Total Expenses</Text>
            <Text style={styles.cardValue}>RM {totalExpenses.toFixed(2)}</Text>
          </View>
        </View>
        ...
        <View style={styles.reportContainer}>
          ...
          <LineChart
            ...
            chartConfig={{
              ...
              propsForLabels: {
                transform: [{ rotate: "-45deg" }], // Rotate all labels diagonally
                textAnchor: "end",
              },
